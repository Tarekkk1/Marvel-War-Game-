...
public class Game {
	...
	public boolean returnFLA() {
		return this.firstLeaderAbilityUsed;
		
	}
	public boolean returnSLA() {
...
public ArrayList<Champion> copyofturnOrder() {
	ArrayList <Champion > a=new ArrayList <>();
	while  (!this.turnOrder.isEmpty()) {
		a.add((Champion) this.turnOrder.remove());
	...
	public void validateCastAbility(Ability a) throws NotEnoughResourcesException, AbilityUseException {
	...
	public Player getPlayerchamp() {
		if (this.getFirstPlayer().getTeam().contains(this.getCurrentChampion()))
				return this.getFirstPlayer();
		else return this.getSecondPlayer();
